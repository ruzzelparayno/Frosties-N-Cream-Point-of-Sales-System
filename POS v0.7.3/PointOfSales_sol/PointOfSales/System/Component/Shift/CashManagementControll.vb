Public Class CashManagementControll
    ...
    Public Sub UpdateShiftSales(ByVal refundValue As Decimal, ByVal discountValue As Decimal, ByVal grossSaleValue As Decimal)
        Try
            ' Update discounts and gross sales only
            Dim currentDiscount As Decimal
            Decimal.TryParse(lbl_sdiscounts.Text.Replace("₱", "").Replace(",", "").Trim(), currentDiscount)
            currentDiscount += discountValue
            lbl_sdiscounts.Text = "₱" & currentDiscount.ToString("N2")

            Dim currentGS As Decimal
            Decimal.TryParse(lbl_gs.Text.Replace("₱", "").Replace(",", "").Trim(), currentGS)
            currentGS += grossSaleValue
            lbl_gs.Text = "₱" & currentGS.ToString("N2")

            ' ⚠ Do NOT update refunds here
            ' lbl_refund1 and lbl_srefunds will be handled in RefundForm directly
